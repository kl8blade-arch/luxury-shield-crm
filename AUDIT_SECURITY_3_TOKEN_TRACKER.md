...
**File Size:** 224 lines  
**Purpose:** Centralized AI call wrapper with rate limiting and token tracking  
**Critical Importance:** Controls all Claude API access, enforces rate limits, tracks token consumption and costs
...
### 1. Admin Bypass (Lines 35, 48, 139)
```typescript
const isAdmin = agentId === ADMIN_ID || !agentId
```
...
When `agentId === 'ee0389f9-6506-4a48-a6f0-6281ade670b9'` OR when `agentId` is null/undefined:
- ALL rate limits are skipped (line 48)
- Token tracking is completely bypassed (line 139)
- AI blocking checks are ignored (line 38)
- No usage logging occurs

**Impact:** Master/Carlos can make unlimited AI calls without any cost tracking or restrictions.

### 2. Rate Limiting Flaw (Lines 48-63)
- Rate limit checking ONLY happens for non-admin agents
- Exception logic on line 53: if rate limit fails AND reason is "no_balance", then `blockAgentAI()` is called
- But admin bypasses this entire check, meaning admin can still use AI even with zero tokens

### 3. API Key Selection Hierarchy (Lines 65-93)
1. Platform key (process.env.ANTHROPIC_API_KEY)
2. Agent's encrypted key (if `uses_own_ai_keys`)
3. Agent's plain key (legacy fallback)
4. Back to platform key

**Issue:** Line 88 checks for presence of `•` character to detect masked keys. This is fragile.

### 4. Logging Exposes Sensitive Information (Line 95)
```typescript
console.log(`[TOKEN-TRACKER] Final API key being used (length: ${apiKey.length}), starts with: ${apiKey.substring(0, 10)}...`)
```

Logs first 10 characters of API key to console. This could be captured in logs and expose the key pattern.

### 5. Cost Calculation (Line 128)
```typescript
const cost = (inputTokens * 0.0000008) + (outputTokens * 0.000004)
```

- Cost per input token: $0.0000008 (Haiku pricing)
- Cost per output token: $0.000004 (Haiku pricing)
- BUT: These are hardcoded prices. If rates change or model changes, cost will be inaccurate
- No charge-back or billing integration shown

### 6. Token Logging Only for Non-Admin (Lines 139-159)
- Admin AI calls are completely invisible to the token_usage table
- Master's actions cannot be audited through token tracking
- No cost tracking for admin

### 7. Agent AI Blocking (Lines 167-189)
- When agent's AI is blocked, they still get an empty response (no error)
- WhatsApp notification is sent, but async and not awaited
- If Twilio request fails, blocking still succeeds silently

### 8. Missing Validation
- No validation that API key is actually valid before using it
- No checking if model exists in Anthropic API
- No timeout on Claude API calls
- No retry logic for transient failures

### 9. Fine-Tuning Model Switch (Lines 97-104)
- If a fine-tuned model is active, it's used for sophia_whatsapp calls
- No audit trail of when/why fine-tuned model switched
- Could be exploited to force use of compromised fine-tuned model

### 10. Table Access Without RLS Check
- Reads `agents`, `token_usage`, `fine_tuning_jobs` tables
- No verification that these tables have Row-Level Security enabled
- Uses SERVICE_ROLE_KEY which bypasses RLS entirely (lines 3-4)
