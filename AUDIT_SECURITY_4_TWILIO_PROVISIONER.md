# SECURITY AUDIT: Twilio Provisioner
## File: src/lib/twilio-provisioner.ts
### Complete Unabridged Source Code

**File Size:** 174 lines  
**Purpose:** Provision Twilio phone numbers for agents (managed or bring-your-own)  
**Critical Importance:** Controls agent phone provisioning and credential management
...
### 1. Master Twilio Credentials (Lines 5-6)
```typescript
const MASTER_SID = process.env.TWILIO_ACCOUNT_SID!
const MASTER_TOKEN = process.env.TWILIO_AUTH_TOKEN!
```
...
These are environment variables that give full access to the master Twilio account:
- Account SID: Uniquely identifies the Twilio account
- Auth Token: Provides authentication for all Twilio API requests
- Exposed in code/logs/monitoring if there's a breach

**Impact:** Anyone with access to these credentials can:
- Create/delete/modify phone numbers
- Send SMS/WhatsApp messages as the company
- Intercept incoming messages
- Manage sub-accounts

### 2. Sub-Account Creation (Lines 33-38)
```typescript
const subaccount = await twilioRequest('/Accounts.json', 'POST', {
  FriendlyName: `SophiaOS-${agentName}-${agentId.slice(0, 8)}`,
})
...
**Issues:**
- No validation of `agentName` before using in FriendlyName
- No size limits on agentName (could be injected with special characters)
- No check if agent already has a sub-account
- No approval workflow or cost controls

### 3. Webhook URL Injection (Lines 57-59)
```typescript
SmsUrl: `${APP_URL}/api/whatsapp/${agentId}`,
```
...
The webhook URL is constructed with:
- `APP_URL` from environment
- `agentId` directly interpolated

**Potential Issues:**
- If agentId contains special characters, it could create an invalid URL
- No validation that this endpoint exists
- All incoming messages for this number will be sent to `/api/whatsapp/{agentId}`
- If agentId is compromised, attacker can redirect to malicious endpoint

### 4. Credential Encryption (Lines 65-67)
```typescript
const { encryptApiKey } = await import('./encryption')
const encToken = encryptApiKey(subaccount.auth_token, agentId)
```
...
**Issues:**
- Uses `agentId` as the encryption key/salt
- If an agent's ID is known, attacker can decrypt their Twilio token
- No separate key management
- Encryption implementation not audited

### 5. Bring-Your-Own Credential Verification (Lines 96-105)
```typescript
if (provider === 'twilio') {
  const url = `https://api.twilio.com/2010-04-01/Accounts/${accountSid}/IncomingPhoneNumbers.json?PhoneNumber=${encodeURIComponent(phoneNumber)}&Limit=1`
...
- Only checks if the phone number exists in the provided account
- Does NOT verify that the credentials are actually valid
- Does NOT verify ownership of the account
- Agent could provide fake credentials and the function would still "succeed"
- No test call to verify credentials work

### 6. Webhook Update Without Rate Limiting (Lines 107-112)
```typescript
await fetch(`https://api.twilio.com/2010-04-01/Accounts/${accountSid}/IncomingPhoneNumbers/${numSid}.json`, {
  method: 'POST',
  ...
...
- No error handling if this request fails
- Silently fails and continues with `return { success: true }`
- Agent could have brought-your-own number but webhook never updated

### 7. Database Upsert Without Deduplication (Lines 68-77, 120-127)
```typescript
await supabase.from('agent_twilio_config').upsert({
  agent_id: agentId,
  mode: 'managed',
...
- Uses `upsert()` which will overwrite existing config
- If agent provisions twice, old sub-account is orphaned (still charges Twilio)
- No cleanup of previous sub-account

### 8. Error Handling is Weak (Lines 84-89)
```typescript
catch (err: any) {
  console.error('[TWILIO-PROVISION]', err.message)
  await supabase.from('tenant_security_events').insert({
...
**Issues:**
- If error insertion fails, silently swallows it
- No retry logic
- Raw error messages are returned to user (could leak system info)
- No cleanup if sub-account is created but number purchase fails

### 9. Decryption Without Validation (Lines 153-170)
```typescript
const { decryptApiKey } = await import('./encryption')
...
**Issues:**
- JSON.parse could throw if token is malformed
- decryptApiKey could fail silently
- No validation that decrypted token is valid format
- Returns token in plain text to caller (could be logged)

### 10. Missing Features
- No rate limiting on provisioning (agent could create unlimited sub-accounts)
- No cost tracking (Twilio charges for sub-accounts and numbers)
- No expiration or auto-cleanup of unused numbers
- No audit trail of who provisioned what when
- No maximum number of numbers per agent
- deprovisionNumber() doesn't validate it actually closed the account

### 11. Environment Variable Assumptions (Line 7)
```typescript
const APP_URL = process.env.NEXT_PUBLIC_APP_URL || 'https://luxury-shield-crm.vercel.app'
```

- Fallback URL is hardcoded
- If environment is misconfigured, webhooks point to production (security risk)
- No validation that APP_URL is HTTPS

### 12. Hardcoded Twilio API Version (Lines 12, 41, 46, 100, 108)
```typescript
const url = `https://api.twilio.com/2010-04-01/Accounts/${sid}${path}`
```

- API version `2010-04-01` is hardcoded
- If Twilio deprecates, system breaks
- No version negotiation or upgrade strategy
